...
SET TERMOUT ON SERVEROUT ON LONG 1000000 LONGCHUNKSIZE 1000000 LINESIZE 90 VERIFY OFF FEEDBACK OFF HEADING OFF
...
PROMPT
prompt &_C_YELLOW in sqlarea.sql &_C_RESET
accept _sql_id PROMPT "&_C_RED Enter sql_id: &_C_RESET" 
accept _interval PROMPT "&_C_RED Enter interval (in secs): &_C_RESET"

SET TERMOUT OFF

BEGIN
  :sql_id := '&_sql_id';
  :interval := to_number('&_interval');
  IF :interval < 0
  THEN
    :interval := 0;
  END IF;
