...
group by  decode(n.wait_class,'User I/O','User I/O', 
                        ...
                        'Scheduler','Scheduler',
                        'Network','Network',
                        'Other', 'other'), BEGIN_TIME, intsize_csec, m.inst_id
union 
SELECT begin_time, 
    intsize_csec/100 AS duration,
    inst_id,
    metric_name , value
  FROM gv$metric   -- statistic values of the last hour captured by AWR infrastructure
...
FROM CPU_WAIT_STAT   
group by begin_time, duration, inst_id
order by inst_id;
